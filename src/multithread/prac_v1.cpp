...
int main(int argc, char const *argv[])
{
    ...
    do{
        double m0, z[2], mom_t[N_Particle][16];
        ...
        for (j=0;j<N_Particle;j++)
        {
            threads.emplace_back(
                [j, &particle, &mom_t]{oper(&particle[j][0], particle[j][1], (int)8*Uniform(1));
                oper(&particle[j][1], particle[j][0], (int)8*Uniform(1));
        ...
        if (z[0] < z[1]) {
            printf("\r %d-st Generation processing. z0=%1.10e  z1=%1.10e", i, z[0], z[1]);
            i++;
            z[1] = z[0];
