        ...
        for (i=0;i<N_Particle;i++)
        {
            ...
            for (k=0;k<9;k++)
            {
                for (l=0;l<9;l++)
                {
                    ...
                    oper(&p_buf[0], p_buf[1], k); oper(&p_buf[1], p_buf[0], l);
                    func(p_buf, mom);
                    op_buf[9*k+l] = lossfunction(moment_real_13, mom);
                } 
            }
            find_min(op_buf, &lossmin);
            p_buf[0] = p[i][0]; p_buf[1] = p[i][1];
            oper(&p_buf[0], p_buf[1], (int)lossmin/9); oper(&p_buf[1], p_buf[0], (int)lossmin%9);
