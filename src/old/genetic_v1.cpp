...
void initPos(double [][2], double, double, double);
...
int main()
{
    ...
    initPos(p_init, moment_real_13[0], moment_real_13[1], 20);
    ...
    for (j=0;j<N_Gen;j++)
    {
        i = 0;
        double z[2];
        printf("\n\n%d-st Generation processing.\n\n", j);
        if (j==0) z[1] = 1.0e50;
        do {
            printf("\r  z0=%5.10f  z1=%5.10f", z[0], z[1]);
...
double rand_normal(double mu, double sigma){
	double z=sqrt( -2.0*log((double) genrand_int32()/4294967296) ) * sin( 2.0*M_PI*(double) genrand_int32()/4294967296 );
	return mu + sigma*z;
}
...
void initPos(double p[][2], double mux, double muy, double sigma)
{
    ...
    for (i=0;i<N_Particle;i++)
    {
        p[i][0] = rand_normal(mux, sigma); // Position of X
        p[i][1] = rand_normal(muy, sigma); // Position of Y
    }
    return;
}
