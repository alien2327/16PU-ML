...
// Evalution function
double lossfunction(double *, double *, double);
double mse(double *, double *);
double ep(double *, double *, double);

void printStat(vector<vector<double>> &p, char fName[20]) {
    int i;
...
void printtime(const chrono::system_clock::time_point& p)
{
...
int main(void)
{
    ...
    vector<vector<double>> particle_init(N_Particle, vector<double>(2, 0));
    vector<vector<double>> particle_better(N_Particle, vector<double>(2, 0));
    vector<vector<double>> particle(N_Particle, vector<double>(2, 0));
    initPos(particle_init, moment_real_13[1], moment_real_13[2]);
    ...
    do{
        ...
        z[0] = lossfunction(moment_real_13, mom, 1.0e-5);
        if (i<10) printf("\r*    00%d-st G:  z1=%1.5e", i, z[1]);
        else if (i>=10 && i<100) printf("\r*    0%d-st G:  z1=%1.8e", i, z[1]);
        else printf("\r*    %d-st G:  z1=%1.5e", i, z[1]);
        if (abs(z[0]) < abs(z[1])) {
            //printf("\r*    %d-st Generation(%d try) : \tz1=%1.8e", i, t, z[1]);
            i++;
            z[1] = z[0];
            particle_better = particle;
            for (k=0;k<16;k++)
            {
                mom_res[k] = mom[k];
            }
    ...
    printStat(particle, ffinal);
    printf("*    Final multipole moment status\n");
    printf("**********************************************************************************\n");
    printf("*\n");
    for (i=0;i<16;i++)
    {
        printf("*    %d\t%1.3e\t%1.3e\t%3.3f%\n", i, moment_real_13[i], mom_res[i], 100*(moment_real_13[i]-mom_res[i])/moment_real_13[i]);
    }
...
void mutation(vector<vector<double>> &p)
{   
    vector<thread> threads;
    for (int i=0;i<N_Particle;i++)
    {
        threads.emplace_back(
            [i, &p]{        
                if (Uniform(1) <= mrate) {
                    double posMax = 50.0, posMin = -50.0;
...
double msle(double *signal, double *simul)
{
    int i;
    double res=0;
    for (i = 5; i < 16; i++)
    {
        res += pow(log10((1+signal[i])/(1+simul[i])),2);
    }
    res /= (double)11;
...
double ep(double *signal, double *simul, double w)
{
    int i=1, ii, iii;
    double res=0.0, norm, skew;
    i=2;
    ii = 2*i - 1;
    iii = 2*i;
    norm = exp(1.0e+2*w*pow(simul[ii] - signal[ii],2));
    skew = exp(1.0e+2*w*pow(simul[iii] - signal[iii],2));
    res += (norm+skew)-2;
    i=3;
    ii = 2*i - 1;
    iii = 2*i;
    norm = exp(1.0e-3*w*pow(simul[ii] - signal[ii],2));
    skew = exp(1.0e-3*w*pow(simul[iii] - signal[iii],2));
    res += (norm+skew)-2;
    i=4;
    ii = 2*i - 1;
    iii = 2*i;
    norm = exp(1.0e-6*w*pow(simul[ii] - signal[ii],2));
    skew = exp(1.0e-6*w*pow(simul[iii] - signal[iii],2));
    res += (norm+skew)-2;
    return res;
}
...
{
    printf("*    Initializing particle position\n");
    printf("**********************************************************************************\n");
    int i;
    for (i=0;i<N_Particle;i++)
    {
...
void func(vector<double> &p, double mom[16]) {
    int i, j;
    double ans;
