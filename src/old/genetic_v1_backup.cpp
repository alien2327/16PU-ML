...
int main()
{
    ...
    int i, j, k;
    double p[N_Particle][2], mom[16];
    double z[N_Particle], cprob[N_Particle];
    ...
    for (j=0;j<N_Gen;j++)
    {
        printf("\r %d-st Generation processing", j);
        for (i=0;i<N_Particle;i++)
        {
            double op_buf[17];
            int lossmin=0;
            for (k=0;k<17;k++)
            {
                rndOper(p[i], k);
                func(p[i], mom);
                op_buf[k] = lossfunction(moment_real_13, mom);
            }
            find_min(op_buf, &lossmin);
            rndOper(p[i], lossmin);
        }
...
{
    int i, ii, iii;
    double w=1.0e-2, res=0.0, norm, skew;
    for (i=0;i<8;i++)
    {
        ii = 2*i;
        iii = ii + 1;
        norm = exp(w*pow(sqrt(simul[ii]*simul[ii]) - sqrt(signal[ii]*signal[ii]),2));
        skew = exp(w*pow(sqrt(simul[iii]*simul[iii]) - sqrt(signal[iii]*signal[iii]),2));
        res += norm + skew;
    }
    res /= (double) 16;
    return res;
...
{
    return 1.0e-30*(mse(signal, simul));//+ep(signal, simul));
}
