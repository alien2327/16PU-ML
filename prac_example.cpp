...
{
    return (mse(signal, simul)+ep(signal, simul, w));
}
...
void initPos(vector<vector<double>> &p, double mux, double muy)
{
    ...
    for (i=0;i<N_Particle;i++)
    {
        p[i][0] = rand_normal(mux, 20); // Position of X
        p[i][1] = rand_normal(muy, 20); // Position of Y
    }
    return;
}
